...
bool rproto_serial_start(rproto_serial *instance) {
    ...
    if (rserial_open(&instance->serial, instance->settings.port_name,
                     instance->settings.baud, instance->settings.mode,
...
bool serial_get_preamble(rproto_serial *instance, unsigned int timeout_ms) {
    ...
    int res = rserial_read(&instance->serial, preamble,
                           sizeof(instance->buf.preamble), timeout_ms * 1000);
    if (res < (int) sizeof(instance->buf.preamble)) {
        return false;
    } else {
        if (instance->buf.preamble == PREAMBLE_REQUEST ||
            instance->buf.preamble == PREAMBLE_UNIQUE_ID ||
            instance->buf.preamble == PREAMBLE_UNIQUE_ID ||
            instance->buf.preamble == PREAMBLE_SET_ID ||
...
bool serial_get_id(rproto_serial *instance, unsigned int timeout_ms) {
    int res = rserial_read(&instance->serial, &instance->buf.id,
                           sizeof(instance->buf.id), timeout_ms * 1000);
...
bool serial_get_proto_message_length(rproto_serial *instance,
                                     unsigned int timeout_ms) {
    int res =
            rserial_read(&instance->serial, &instance->buf.payload_length,
                         sizeof(instance->buf.payload_length), timeout_ms * 1000);
...
bool serial_get_payload(rproto_serial *instance, unsigned int timeout_ms) {
    int res = rserial_read(&instance->serial, instance->buf.payload,
                           instance->buf.payload_length, timeout_ms * 1000);
...
bool serial_get_crc(rproto_serial *instance, unsigned int timeout_ms) {
    int res = rserial_read(&instance->serial, (uint8_t *) &instance->buf.crc,
                           sizeof(instance->buf.crc), timeout_ms * 1000);
...
void encode_payload_and_copy_to(rproto_packet *src_packet,
                                rproto_packet *dest_packet) {
    dest_packet->payload_length = (uint8_t) b64_decode(
            src_packet->payload, src_packet->payload_length, dest_packet->payload);
}

void decode_payload_and_copy_to(rproto_packet *src_packet,
                                rproto_packet *dest_packet) {
    dest_packet->payload_length = (uint8_t) b64_encode(
            src_packet->payload, src_packet->payload_length, dest_packet->payload);
}

uint16_t computing_crc_of_packet(rproto_packet *packet) {
    size_t size = sizeof(packet->preamble) + sizeof(packet->id) +
                  sizeof(packet->payload_length) + packet->payload_length;
    uint16_t result = crc16_ccitt((char *) packet, (int) size);
    return result;
}

int rproto_serial_setup(rproto_serial *instance, char *port_name, int baud,
                        char *mode, int flowctrl) {
...
bool rproto_serial_get_packet(rproto_serial *instance, rproto_packet *packet,
                              unsigned int timeout_ms) {
    ...
    /** get response */
    bool got_client_id_and_length =
            serial_get_proto_message_length(instance, timeout_ms);
...
bool rproto_serial_send_packet(rproto_serial *instance, rproto_packet *packet) {
    ...
    if (packet->payload_length > PROTO_MAX_PAYLOAD_LENGTH) {
        RLOG_ERROR("payload length is too long MAX VALUE is %d",
    ...
    memcpy(base64_payload_packet, (uint8_t *) packet,
           sizeof(packet->preamble) + sizeof(packet->id));
    decode_payload_and_copy_to(packet, base64_packet);

    size_t size = sizeof(packet->preamble) + sizeof(packet->id) +
                  sizeof(packet->payload_length) + base64_packet->payload_length;
